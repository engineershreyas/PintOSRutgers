...
thread_set_priority (int new_priority)
{
  ...
  original_thread = thread_current()->priority; 
  //current thread is reassigned priority value (below)
  int mod_thread = thread_current ()->priority = new_priority; 
  //prioritize again
  priority_reinstate();
  //if-statement to compare new priority of current_thread to first element in the stack (below)
  if(original < mod_thread) { //if current thread no longer has higher priority
      priority_donate();

  }

  if (original_thread > mod_thread)
    {
...
// //compairs 2 threads by the "ticksToWait" member fucntion
